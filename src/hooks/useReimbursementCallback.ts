...
export function useMintIouDeiCallback(amountIn: string): {
  state: TransactionCallbackState
  callback: null | (() => Promise<string>)
  error: string | null
} {
  ...
  const constructCall = useCallback(() => {
    try {
      ...
      return {
        address: reimbursementContract.address,
        calldata: reimbursementContract.interface.encodeFunctionData('mintIOUDEIToken', args) ?? '',
        value: 0,
      }
    } catch (error) {
      return {
  ...
  }, [account, library, reimbursementContract, amountIn])
  ...
  return useMemo(() => {
    ...
    const summary = `Minted IOUDEI Token`

    return {
      state: TransactionCallbackState.VALID,
      error: null,
      callback: () => createTransactionCallback('MintIOUDei', constructCall, addTransaction, summary, account, library),
    }
  }, [account, chainId, library, reimbursementContract, constructCall, addTransaction])
}

export function useRedeemIouDeiCallback(amountIn: string): {
  state: TransactionCallbackState
  callback: null | (() => Promise<string>)
  error: string | null
} {
  ...
  return useMemo(() => {
    ...
    const summary = `Redeem IOUDEI Token`

    return {
      state: TransactionCallbackState.VALID,
      error: null,
      callback: () =>
        createTransactionCallback('RedeemIOUDei', constructCall, addTransaction, summary, account, library),
    }
  }, [account, chainId, library, reimbursementContract, constructCall, addTransaction])
}
